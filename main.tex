...
%% preprint: removes line numbers, switch to two-columns, print author credits

%\documentclass[breakmath,anonymous]{seismica}
\documentclass[breakmath]{seismica}

\title{Your article title goes here}

%% Will not be printed if anonymous option ON
%% Orcid will only be printed if manuscript is accepted
...
%% Author CRediT roles (will only be printed if manuscript is accepted)
...
\begin{document}
	...
	\makeseistitle{%
		...
		%            An optional second-languag abstract may go here. This should be no longer than 200 words. Please replace "Résumé" and "french" with the appropriate term and language.
		%    \end{summary}
		%    \end{french}
